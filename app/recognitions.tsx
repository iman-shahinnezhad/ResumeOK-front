...
export default function RecognitionsScreen() {
  ...
  const loadProfileData = async () => {
    try {
      ...
      console.log('Error loading recognitions profile data:', e);
    }
  };

  const saveRecognitions = async (updatedList: RecognitionItem[]) => {
    try {
      const path = `${FileSystem.documentDirectory}user_onboarding_profile.json`;
      const updatedProfile = { ...(profile || {}), recognitions: updatedList };
      setProfile(updatedProfile);
      setRecognitions(updatedList);
      await FileSystem.writeAsStringAsync(path, JSON.stringify(updatedProfile));
    } catch (e) {
      console.log('Error saving recognitions list:', e);
    }
  };
  ...
  const handleOpenEdit = (item: RecognitionItem) => {
    setEditingId(item.id);
    setRecognitionTitle(item.recognitionTitle || '');
    setRecognizedBy(item.recognizedBy || '');
    setDate(item.date || 'May 2024');
    setDescription(item.description || '');
    setRecognitionUrl(item.recognitionUrl || '');
    setIsModalOpen(true);
  };
  ...
  const handleSaveRecognition = () => {
    if (!recognitionTitle.trim() || !recognizedBy.trim()) return;

    const itemData: RecognitionItem = {
      id: editingId || Date.now().toString(),
      recognitionTitle: recognitionTitle.trim(),
      recognizedBy: recognizedBy.trim(),
      date: date.trim(),
      description: description.trim(),
      recognitionUrl: recognitionUrl.trim(),
    };

    if (editingId) {
      const updatedList = recognitions.map((r) => (r.id === editingId ? itemData : r));
      saveRecognitions(updatedList);
    } else {
      saveRecognitions([itemData, ...recognitions]);
    }

    setIsModalOpen(false);
  };

  const handleMenuPress = (item: RecognitionItem, index: number) => {
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        {
          options: ['Edit Recognition', 'Delete', 'Cancel'],
          destructiveButtonIndex: 1,
          cancelButtonIndex: 2,
          title: item.recognitionTitle,
          message: item.recognizedBy,
        },
        (buttonIndex) => {
          if (buttonIndex === 0) {
            handleOpenEdit(item);
          } else if (buttonIndex === 1) {
            const updated = recognitions.filter((_, i) => i !== index);
            saveRecognitions(updated);
          }
        }
      );
    } else {
      Alert.alert(item.recognitionTitle, 'Choose action', [
  ...
  const isFormValid = recognitionTitle.trim().length > 0 && recognizedBy.trim().length > 0;
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.explanationBanner}>
          <Ionicons name="star-outline" size={24} color="#2563EB" style={{ marginRight: 10 }} />
          <View style={{ flex: 1 }}>
            <Text style={styles.bannerTitleText}>Recognition & Endorsements</Text>
            <Text style={styles.bannerSubtitleText}>
              Add public recognitions, peer awards, media mentions, or industry honors.
            </Text>
          </View>
        </View>

        {/* + ADD RECOGNITION OUTLINE BUTTON */}
        <TouchableOpacity
        ...
        >
          ...
          <Text style={styles.addOutlineBtnText}>ADD RECOGNITION</Text>
        </TouchableOpacity>

        {/* LIST OF SAVED RECOGNITIONS */}
        {recognitions.map((item, index) => (
          <View key={item.id || index} style={styles.recCard}>
            <View style={styles.cardHeader}>
              <View style={{ flex: 1 }}>
                <Text style={styles.recTitleText}>{item.recognitionTitle}</Text>
                <Text style={styles.recognizedByText}>Recognized by {item.recognizedBy}</Text>
              </View>
            ...
            {item.recognitionUrl ? (
              <View style={styles.linkPill}>
                ...
                <Text style={styles.linkText} numberOfLines={1}>{item.recognitionUrl}</Text>
              </View>
            ) : null}
          </View>
        ))}
      ...
      >
        <View style={[styles.cleanFullPageModal, { paddingTop: insets.top + 10, paddingBottom: Math.max(insets.bottom + 10, 20) }]}>
          ...
          >
            {/* 1. Recognition Title * */}
            {renderFloatingInput('Recognition Title *', recognitionTitle, setRecognitionTitle, 'recognitionTitle')}

            {/* 2. Recognized By * */}
            {renderFloatingInput('Recognized By * (e.g. Forbes, TechCrunch, CEO)', recognizedBy, setRecognizedBy, 'recognizedBy')}
            ...
            {/* 5. Recognition URL (Optional) */}
            {renderFloatingInput('Recognition URL (Optional)', recognitionUrl, setRecognitionUrl, 'recognitionUrl')}
          </ScrollView>
